-- Insert 100 food products for Demo Food Store (run in Supabase SQL Editor)
-- Deletes existing products for the food project, then inserts 100 (có project_type = 'food').
...
begin
  ...
  where project_type = 'food'
    and (name = 'Demo Food Store' or name = 'Food Store')
  order by created_at desc
  limit 1;

  if pid is null then
    raise notice 'No food project (Demo Food Store or Food Store) found. Create one in Dashboard or run seed.sql first.';
    return;
  end if;
  ...
  raise notice 'Inserted 100 food products for Demo Food Store.';
end $$;
